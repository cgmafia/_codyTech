...
for page in range(0, 900):
    n_pages += 1
    sapo_url = 'https://casa.sapo.pt/Venda/Apartamentos/?sa=11&lp=10000&or=10' + \
        '&pn='+str(page)
    ...
    if house_containers != []:
        for container in house_containers:
            ...
            price_ = [int(price[s])
                      for s in range(0, len(price)) if price[s].isdigit()]
            price = ''
            for x in price_:
                price = price+str(x)
            ...
            # Zone
            location = container.find_all(
                'p', class_="searchPropertyLocation")[0].text
            ...
            # Creation date
            date = pd.to_datetime(container.find_all(
                'div', class_="searchPropertyDate")[0].text[21:31])
            created.append(date)

            # Description
            desc = container.find_all('p', class_="searchPropertyDescription")[
                0].text[7:-6]
            descriptions.append(desc)

            # url
            link = 'https://casa.sapo.pt/' + \
                container.find_all('a')[0].get('href')[1:-6]
            urls.append(link)
